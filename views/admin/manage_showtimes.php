...
$shows = $conn->query("SELECT s.*, m.title FROM showtimes s JOIN movies m ON s.movie_id = m.id ORDER BY s.start_time ASC");

// Khởi tạo thông báo thành công và lỗi
...
<body>
    ...
    <div class="container my-5">
        <h2 class="text-center admin-heading">📅 Quản lý Suất Chiếu</h2>
        <?php if ($success): ?>
        ...
        <?php endif; ?>
        <a href="add_showtime.php" class="btn btn-add-show"><i class="fas fa-plus"></i> Thêm Suất Chiếu</a>
                ...
                <tbody>
                    <?php while ($row = $shows->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a href="edit_showtime.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-edit"><i
                                        class="fas fa-pen"></i> Sửa</a>
                                <a href="../../handle/delete_showtime_process.php?id=<?= $row['id'] ?>"
                                    class="btn btn-sm btn-delete"
                                    onclick="return confirm('Bạn có chắc chắn muốn xóa xuất chiếu này không?')"><i
